...
import pascal.taie.analysis.pta.plugin.taint.EnhanceTaintAnalysis;
...
import pascal.taie.analysis.pta.toolkit.pruning.DistancePruning;
...
public class PointerAnalysis extends ProgramAnalysis<PointerAnalysisResult> {
    ...
    public static CompositePlugin lastPlugin;
            ...
            if (advanced.equals("collection")) {
                ...
                if (advanced.startsWith("scaler")) {
                    ...
                    heapModel = Timer.runAndCount(() -> Mahjong.run(preResult, options),
                            "Mahjong", Level.INFO);
                } else if (advanced.startsWith("pruning")) {
                    List<EnhanceSink> enhanceSinks = new ArrayList<>();
                    lastPlugin.getAllPlugins().stream().forEach(plugin -> {
                        if(plugin instanceof EnhanceTaintAnalysis enhanceTaintAnalysis) {
                            enhanceTaintAnalysis.getSinkHandler().getSinks().forEach(enhanceSink -> enhanceSinks.add(enhanceSink));
                        }
                    });
                    selector = Timer.runAndCount(() -> ContextSelectorFactory
                            .makePruningSelector(DistancePruning.run(preResult, options, enhanceSinks)),
                            "Pruning", Level.INFO);
                } else {
