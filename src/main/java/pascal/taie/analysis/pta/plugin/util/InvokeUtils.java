...
public final class InvokeUtils {
    ...
    public static boolean isCompatiable(Invoke callSite, JMethod callee) {
        return !callSite.isDynamic()
                && ((callSite.isStatic() && callee.isStatic()) || (!callSite.isStatic() && !callee.isStatic()))
//                && ((callSite.getDef().isPresent() && !callee.getReturnType().equals(VoidType.VOID)) || (callSite.getDef().isEmpty() && callee.getReturnType().equals(VoidType.VOID)))
                && callSite.getInvokeExp().getArgCount() <= callee.getParamCount();
    }
}
