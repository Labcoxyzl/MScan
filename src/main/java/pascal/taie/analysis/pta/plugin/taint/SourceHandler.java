...
class SourceHandler extends OnFlyHandler {
    ...
    static Logger logger = LogManager.getLogger(SourceHandler.class);
    ...
    private void processCallSource(Context context, Invoke callSite, CallSource source) {
        ...
        if (var.getType() instanceof ArrayType) {
            taint = manager.makeTaint(sourcePoint, source.type(), true);
        } else {
            taint = manager.makeTaint(sourcePoint, source.type());
        }
