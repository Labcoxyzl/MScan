...
public class DefaultSolver implements Solver {
        ...
        if (!diff.isEmpty()) {
            pointerFlowGraph.getOutEdgesOf(pointer).forEach(edge -> {
                ...
                edge.getTransfers().forEach(transfer -> {
                    if (edge.source() instanceof CSVar csVar) {
                        if (csVar.getVar().getMethod().getDeclaringClass().getName().startsWith("org.springframework.web.util.")) {
                            return;
                        }
                    }
                    PointsToSet results = transfer.apply(edge, diff);
                    addPointsTo(target, results);
                });
            });
        }
        return diff;
        ...
        for (StoreArray store : var.getStoreArrays()) {
            ...
            if (propTypes.isAllowed(rvalue)) {
                ...
                pts.forEach(array -> {
                    ...
                    } else if (array.getObject() instanceof ConstantObj constantObj) {
                        if (constantObj.getAllocation() instanceof StringLiteral stringLiteral) {
                            if (stringLiteral.getString().equals("upload") || stringLiteral.getString().equals("package")) {
                                ArrayIndex arrayIndex = csManager.getArrayIndex(array);
                                // we need type guard for array stores as Java arrays
                                // are covariant
                                addPFGEdge(from, arrayIndex,
    ...
    public void addPFGEdge(Pointer source, Pointer target, FlowKind kind,
        ...
        if (edge != null && edge.addTransfer(transfer)) {
            PointsToSet pointsToSet = getPointsToSetOf(source);
            if (source instanceof ArrayIndex arrayIndex)
                pointsToSet.addObject(arrayIndex.getArray());
            PointsToSet targetSet = transfer.apply(edge, pointsToSet);
